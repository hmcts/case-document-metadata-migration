...
insert into all_events(jurisdiction,case_type_id,case_reference,case_id,case_event_id,document_id,document_url,event_timestamp,doc_present)
              select
...
from case_event ce,
              (select distinct jurisdiction,case_type_id,case_reference,case_id,document_id,document_url
                from doc_events) cd
                where ce.case_data_id = cd.case_id;

insert into doc_store_export(jurisdiction,case_type_id,case_reference,document_id,document_url,event_timestamp)
        select ae1.jurisdiction,ae1.case_type_id,ae1.case_reference,ae1.document_id,ae1.document_url,ae1.event_timestamp
        from doc_events ae1
        inner join
        (

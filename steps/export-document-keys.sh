...
#    export PGPASSWORD="$DEFINITION_STORE_PASS"
    pg_dump -h "$DEFINITION_STORE_HOST" -p "$DEFINITION_STORE_PORT" -d "$DEFINITION_STORE_NAME" -U "$DEFINITION_STORE_USER" -W -t document_keys > tmp/document_keys.tbl
    unset PGPASSWORD
fi
...
#    export PGPASSWORD="$DEFINITION_STORE_TEMP_PASS"
    pg_dump -h "$DEFINITION_STORE_TEMP_HOST" -p "$DEFINITION_STORE_TEMP_PORT" -d "$DEFINITION_STORE_TEMP_NAME" -U "$DEFINITION_STORE_TEMP_USER" -W -t document_keys > tmp/document_keys.tbl
    unset PGPASSWORD
fi

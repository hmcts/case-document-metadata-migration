...
if [ $OPERATION$DBTYPE = "exportdocumentidsrealtime" ]; then
    ...
    psql -h "$DATA_STORE_HOST" -p "$DATA_STORE_PORT" -d "$DATA_STORE_NAME" -U "$DATA_STORE__USER" -w  -c "DROP TABLE IF EXISTS staging;" 2>&1 > /dev/null
    psql -h "$DATA_STORE_HOST" -p "$DATA_STORE_PORT" -d "$DATA_STORE_NAME" -U "$DATA_STORE_USER" -w -v FROM_DATE="'${FROM_DATE}'" -v JURISDICTION="'${JURISDICTION}'" -f scripts/migrate-staging.sql 2>&1 > /dev/null
    unset PGPASSWORD
fi
...
if [ $OPERATION$DBTYPE = "exportdocumentidssnapshotdb" ]; then
    ...
    psql -h "$DATA_STORE_TEMP_HOST" -p "$DATA_STORE_TEMP_PORT" -d "$DATA_STORE_TEMP_NAME" -U "$DATA_STORE_TEMP_USER" -w  -c "DROP TABLE IF EXISTS staging;" 2>&1 > /dev/null
    psql -h "$DATA_STORE_TEMP_HOST" -p "$DATA_STORE_TEMP_PORT" -d "$DATA_STORE_TEMP_NAME" -U "$DATA_STORE_TEMP_USER" -w -v FROM_DATE="'${FROM_DATE}'" -v JURISDICTION="'${JURISDICTION}'" -f scripts/migrate-staging.sql 2>&1 > /dev/null
    unset PGPASSWORD
fi

...
##
# Load the snapshots into local temporary databases
##
unset PGPASSWORD
...
psql sslmode=true -h "$DATA_STORE_TEMP_HOST" -p "$DATA_STORE_TEMP_PORT" -d "$DATA_STORE_TEMP_NAME" -U "$DATA_STORE_TEMP_USER" -w -f scripts/drop-tables-tempdb.sql 2>&1 > /dev/null
echo "STEP 1 DONE"
unset PGPASSWORD

echo "STEP 2: LOAD ALL TABLES FROM SNAPSHOT IN TO TEMPDB"
export PGPASSWORD="$DATA_STORE_TEMP_PASS"
psql sslmode=true -h "$DATA_STORE_TEMP_HOST" -p "$DATA_STORE_TEMP_PORT" -d "$DATA_STORE_TEMP_NAME" -U "$DATA_STORE_TEMP_USER" -w -f "$DATA_STORE_SNAPSHOT"
echo "STEP 2 DONE"
unset PGPASSWORD

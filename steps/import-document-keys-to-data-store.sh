...
if [ $OPERATION$DBTYPE = "exportdocumentidsrealtime" ]; then
    ...
    psql -h "$DATA_STORE_HOST" -p "$DATA_STORE_PORT" -d "$DATA_STORE_NAME" -U "$DATA_STORE_USER" -w  -c "DROP TABLE IF EXISTS document_keys;" 2>&1 > /dev/null
    psql -h "$DATA_STORE_HOST" -p "$DATA_STORE_PORT" -d "$DATA_STORE_NAME" -U "$DATA_STORE_USER" -w  < tmp/document_keys.tbl 2>&1 > /dev/null
    unset PGPASSWORD
fi
...
if [ $OPERATION$DBTYPE = "exportdocumentidssnapshotdb" ]; then
    ...
    psql -h "$DATA_STORE_TEMP_HOST" -p "$DATA_STORE_TEMP_PORT" -d "$DATA_STORE_TEMP_NAME" -U "$DATA_STORE_TEMP_USER" -w  -c "DROP TABLE IF EXISTS document_keys;" 2>&1 > /dev/null
    psql -h "$DATA_STORE_TEMP_HOST" -p "$DATA_STORE_TEMP_PORT" -d "$DATA_STORE_TEMP_NAME" -U "$DATA_STORE_TEMP_USER"  < tmp/document_keys.tbl 2>&1 > /dev/null
    unset PGPASSWORD
fi

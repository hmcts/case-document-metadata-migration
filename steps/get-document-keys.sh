...
if [ $OPERATION$DBTYPE = "exportdocumentidssnapshotdb" ]; then
    ...
    psql  -h "$DEFINITION_STORE_TEMP_HOST" -p "$DEFINITION_STORE_TEMP_PORT" -d "$DEFINITION_STORE_TEMP_NAME" -U "$DEFINITION_STORE_TEMP_USER" -w -v JURISDICTION="'${JURISDICTION}'" -f scripts/get-document-keys.sql
    unset PGPASSWORD
fi
